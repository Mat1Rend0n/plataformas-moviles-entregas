...
function calcularPromedio(listaDeNumeros) {
    ...
    {
        tot=tot+listaDeNumeros[i];
    }
    let calcularPromedio=tot/listaDeNumeros.length;
}
...
function encontrarPalabraMasCorta(listaDePalabras) {
    let palabraMini = listaDePalabras[0];
    for(let i=0; i < listaDePalabras.length; i++)
    {
        if(palabraMini.length < listaDePalabras[i])
        {
            palabraMini = listaDePalabras[i];
        }
    }
    return palabraMini;
}
...
function enumerarLista(listaDePalabras) {
    ...
    return palabra + `y ${listaDePalabras[listaDePalabras.length - 1]}.`;
}
